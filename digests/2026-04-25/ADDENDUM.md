...
## Addendum 15 — Window 09:06Z → 09:38Z

### codex (openai/codex)

- Silent in window. The MCP/policy/permissions surface (synthesis #72: #19392 `bolinfest`, #19524 `aibrahim-oai`, #19537 `mzeng-openai`) has now had **zero codex-side activity for ~73 minutes** since the 08:25Z cluster. Synthesis #72 prediction #2 ("contributor-class follow-up within 30 minutes") **firmly falsified** at this point. The maintainer-class convergence at 08:25Z stands as a closed event, not the leading edge of a sustained work-stream.

### litellm (BerriAI/litellm)

- **[BerriAI/litellm#26498]** — OPENED 09:16:39Z, refreshed 09:23:20Z by `kiyeonjeon21`, head SHA `46509bf2e9d0`. *"fix(auth): apply temp_budget_increase on cache-hit path."* New auth/budget surface PR. **First litellm new-open in ~2h35m** (last open #26496 `yuneng-berri` at 06:41Z). Open → first iteration in ~6m43s (sub-7-minute self-iteration cadence).
- **[BerriAI/litellm#25609]** — refreshed 09:24:20Z by `kiyeonjeon21`, head SHA `4689447416c2`. *"fix(proxy): update existing budget row in /team/member_update instead of creating new."* Long-tail PR (created 2026-04-12, ~13 days old). **Refreshed ~1m00s after #26498's last refresh** by the same author on an adjacent surface (both budget-row management; #25609 on `/team/member_update` write-path, #26498 on auth cache-hit read-path).
- **Same-author adjacent-surface PR doublet** (`kiyeonjeon21` touches #26498 then #25609 within 60 seconds, both on the budget-management surface across read/write boundaries). Same shape as synthesis #47 (same-author rapid-fire doublet on adjacent surfaces) but with a **13-day-old long-tail leg** paired with a freshly-opened leg — a *cross-temporal* same-author doublet, not the prior same-day variant. Anchor for **synthesis #75**.
- **`yuneng-berri` cascade**: still silent (~2h57m dormant). The cascade-termination reading from Addendum 14 holds; litellm's open-velocity has **handed from `yuneng-berri` to `kiyeonjeon21`** with a ~2h35m gap between the two authors' active windows. No author overlap, distinct surface focus (yuneng: xAI/Grok provider; kiyeonjeon21: auth/budget proxy).

### opencode (anomalyco/opencode)

- **[anomalyco/opencode#24277]** — OPENED 09:29:57Z, **CLOSED 09:31:56Z** by `terisuke` (`Terada Kousuke`), head SHA `ac3d7d7de4c5`. *"[codex] Fix team run preflight aborts."* **Sub-2-minute self-close** (1m59s open→close, never refreshed). Title carries `[codex]` prefix despite being filed in opencode — cross-namespace title-prefix anomaly: the author appears to be using `[codex]` as an internal scope tag inside the opencode repo (a labeling convention previously seen on #19524 `aibrahim-oai` codex-side). Also matches **synthesis #59** (sub-15-minute self-close on flaky-test surface) as a *much-faster* exemplar — sub-2-minute is the fastest self-close observed in this corpus.
- **[anomalyco/opencode#24222]** — refreshed 09:30:11Z by `thdxr` (`Dax`). *"fix permission config order."* Maintainer-class refresh on permission-config surface. Last opened 2026-04-24T22:29:49Z (~11h old). First `thdxr` activity tracked in many ticks.
- **[anomalyco/opencode#22296]** — refreshed 09:31:53Z by `Daviey` (`Dave Walker`). *"fix: authoritative managed-settings.json applied after all user config."* Long-tail PR (created 2026-04-13, ~12 days). Configuration-precedence surface — **structurally adjacent to #24222 thdxr permission-config-order** (both touch config-application ordering).
- **[anomalyco/opencode#20467]** — refreshed 09:36:11Z by `kkugot` (`Kostiantyn Kugot`). *"fix: blank assistant text - finish-reason regression after AI SDK v6 migration when any MCP is on."* Long-tail PR (created 2026-04-01, ~24 days). MCP/AI-SDK-v6 regression surface.
- **[anomalyco/opencode#15603]** — refreshed 09:22:49Z by `aklajnert` (`Andrzej Klajnert`). *"fix(question): implement recovery for pending questions when app starts."* Deep long-tail PR (created 2026-03-01, ~55 days old). Question-recovery surface.
- **opencode permission/config-application convergence**: #24222 (`thdxr` permission config order, refreshed 09:30:11Z) and #22296 (`Daviey` managed-settings authority, refreshed 09:31:53Z) — **two PRs on the config-application-ordering surface refreshed within 1m42s of each other by two distinct authors**. One is maintainer (`thdxr`), one is contributor (`Daviey`). Surface family: *which config wins in what order*. Anchor for **synthesis #76**.
- **opencode tick-15 activity profile**: 5 PRs touched (1 open+close in 119s, 4 long-tail refreshes spanning ages 11h / 12d / 24d / 55d). **Zero net new opens that survived the tick** (#24277 self-closed within window). The 3-author docs-cluster of tick 14 (synthesis #73) has zero follow-up activity in tick 15.

### ollama (ollama/ollama)

- Silent in window. #15808 (`famasoon` Scheduler) and #15809 (`stationeros` create-blob-prune) both dormant since tick-14 boundary. Ollama's velocity-leadership of ticks 12–13 has now fully handed to opencode for two consecutive ticks (14, 15) — synthesis #74's leadership-rotation reading **continues**, but the pattern is now "opencode-sustained for ≥2 ticks" rather than the sub-2-tick rotation cadence #74 hypothesized. This **softly falsifies synthesis #74's "rotation cadence is sub-hourly" framing** and supports the "sustained leadership during post-release window" alternative framing #74 listed as falsification path #2.

### cline (cline/cline)

- **cline/cline#10403** (`abliteration-ai` vendor-self-onboarding) — silent in window. Now ~67m+ since last refresh (08:30:15Z). The cadence trace is 22m → 43m → ≥67m — **monotonically decelerating and now exceeded the 60m threshold flagged in Addendum 14**. Synthesis #71's "sub-hourly" framing is now **falsified**; the more accurate reading is "decelerating self-iteration on a vendor-self-onboarding PR with zero maintainer interaction" — the cadence-deceleration shape itself is the new signal.

### crush / continue / OpenHands / aider / sst-opencode / browser-use

- All silent in window. crush dormancy now ~8h25m. continue + litellm `FuturMix` dual-PR coordination dormancy now ~8h+. OpenHands silent ~2h35m. browser-use no events in window.

---

### What changed in window 09:06Z → 09:38Z (Addendum 15)

1. **litellm same-author cross-temporal doublet**: `kiyeonjeon21` touches #26498 (fresh open, auth cache-hit budget) and #25609 (13-day-old long-tail, proxy budget-row write) within 60 seconds, both on the budget-management surface across read/write boundaries. **First cross-temporal same-author doublet observed today** — distinguishing it from synthesis #47's same-day variant. Anchor for **synthesis #75**.
2. **opencode config-application-ordering two-author convergence**: #24222 (`thdxr` permission-config-order) and #22296 (`Daviey` managed-settings authority) refreshed within 1m42s, both on *which-config-wins-in-what-order* surface. Maintainer + long-tail-contributor pairing on a single surface family. Anchor for **synthesis #76**.
3. **Sub-2-minute self-close record**: opencode #24277 (`terisuke`, *"[codex] Fix team run preflight aborts"*) opened 09:29:57Z, closed 09:31:56Z — **1m59s** open-to-close. New fastest-self-close exemplar in this corpus, beating synthesis #59's "sub-15-minute" boundary by a full order of magnitude. The `[codex]` title-prefix-in-opencode is also a cross-namespace tagging anomaly worth tracking.
4. **First non-zero merge attempt in 5 ticks**: still **zero merges across all 9 tracked repos** (Addendums 11–15, ~152 continuous minutes of zero merge-absorption). Synthesis #74's predicted "absorption catch-up tick with 3+ merges in 20m" has not occurred; the open-vs-merge gap continues widening. The 5-tick zero-merge regime is now the longest observed today.
5. **Synthesis #71 falsified, synthesis #72 prediction #2 firmly falsified, synthesis #74 framing partially falsified**: three synthesis revisions in a single tick. Specifically — #71's "sub-hourly self-iteration cadence" is broken by cline #10403's deceleration to ≥67m; #72's "30-minute contributor-class follow-up" deadline expired ~73 minutes ago; #74's "rotation cadence is sub-hourly" framing replaced by "sustained-leadership-during-post-release-window" as opencode now leads for tick 14+15.
6. **Author-set turnover**: tick-15 authors (`kiyeonjeon21`, `terisuke`, `thdxr`, `Daviey`, `kkugot`, `aklajnert`) — **zero overlap with tick-14 authors** (`wangzexi`, `kyuna0312`, `JasonZhaoWW`, `stationeros`). **Three consecutive ticks with full author-set turnover** (12→13→14→15). Reinforces synthesis #70's cross-repo-coupling-is-timing-only finding at the within-repo level too: opencode's tick-14 docs-class cohort and tick-15 config-class cohort share zero authors.
7. **opencode signal-class shift within sustained leadership**: tick-14 was **docs-class** (3 docs/i18n PRs by 3 authors); tick-15 is **config/permission-application-class** (#24222, #22296 paired on config ordering) plus a sub-2m self-close (#24277). Same repo, sustained leadership, but **completely different surface family** between consecutive opencode-led ticks. Suggests post-release downstream surfacing is multi-surface, not single-surface — strengthens synthesis #74's release-recency-driven framing.
8. **litellm velocity baton handoff**: `yuneng-berri` (silent ~2h57m) → `kiyeonjeon21` (active in tick 15). Distinct authors, distinct surfaces (provider integration → auth/budget proxy), ~2h35m gap. The litellm repo's velocity is **author-rotating** even as the repo itself stays active — a sub-pattern that does not appear in opencode (which has had >5 distinct active authors in the last 2 ticks).
